...
impl PyRepo {
    ...
    fn __str__(&self) -> PyResult<String> {
        ...
        let last_ts = summary
            .last_commit_time
            .and_then(|t| DateTime::<Utc>::from_timestamp(t, 0).map(|dt| dt.format("%Y-%m-%d %H:%M").to_string()))
...
fn summarize_repo(repo: &Repository) -> PyResult<PyRepoSummary> {
    let path = repo
        .path()
        .parent()
    ...
    if let Ok(head_ref) = repo.head() {
        ...
        if let Some(target) = head_ref.target() {
            ...
            let mut revwalk = repo.revwalk().map_err(|err| py_git_err("failed to create revwalk", err))?;
            revwalk
                .push(target)
                .map_err(|err| py_git_err("failed to start revwalk from HEAD", err))?;
            for oid_result in revwalk {
                let oid = oid_result.map_err(|err| py_git_err("failed to read oid during summary", err))?;
                commits += 1;
...
fn estimate_repo_size_kb(repo: &Repository) -> PyResult<usize> {
    let odb = repo.odb().map_err(|err| py_git_err("failed to open odb", err))?;
    let mut total: u64 = 0;
    odb.foreach(|oid| {
